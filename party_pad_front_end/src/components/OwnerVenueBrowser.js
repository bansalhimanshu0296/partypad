...
import OwnerVenues from "./OwnerVenues";
...
export default function OwnerVenueBrowser() {
    ...
    useEffect(() => {
        ...
        setCityResult("")
        setSearchValue("")
        axios.post(process.env.REACT_APP_VM_IP + "/app/Fetch_Owner_Venues", {
    ...
    const onRemoveVenue = (venue_id) => {
        const body = {
            email: localStorage.getItem('user'),
            venueId: venue_id
        }
        axios.post(process.env.REACT_APP_VM_IP + "/app/Remove_Venue", body).then(res => {
            if (res.data.status === "OK") {
                axios.post(process.env.REACT_APP_VM_IP + "/app/Fetch_Owner_Venues", {
                    email: localStorage.getItem('user'),
                    page: paginationPage,
                ...
                <Grid item xs={12} sm={12}>
                    <Box style={{ height: "68vh", overflowY: "scroll" }}>
                        <Container component="main" maxWidth="lg">
                            <CssBaseline />
                            {data.length !== 0 && <OwnerVenues data={data} onRemoveVenue={onRemoveVenue}/>}
