...
export default function PasswordRecovery(props) {
  ...
  const onSubmit = () =>{
    if(isCaptchaVerfied || captchaRef === null){
        axios.post("http://"+process.env.REACT_APP_VM_IP+":8000/app/Recover_Password",{
          email: userName
        }).then(res=>{
...
const onCheckSubmit = () =>{
  ...
  axios.post("http://"+process.env.REACT_APP_VM_IP+":8000/app/Verify_Email",{
          email: userName,
          code: number
        }).then(res=>{
          if(res.data.status === "OK"){
...
const onSubmitPassword =() =>{
  ...
  axios.post("http://"+process.env.REACT_APP_VM_IP+":8000/app/Update_Password",{
          email: userName,
