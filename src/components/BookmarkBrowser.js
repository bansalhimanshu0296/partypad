...
import BookmarkVenues from "./BookmarkVenues";
...
export default function BookmarkBrowser() {
    ...
    useEffect(() => {
        ...
        setCityResult("")
        setSearchValue("")
        axios.post(process.env.REACT_APP_VM_IP + "/app/Fetch_Bookmarked_Venues", {
    ...
    const onRemoveBookmark = (venue_id) => {
        const body = {
            email: localStorage.getItem('user'),
            venueId: venue_id
        }
                ...
                <Grid item xs={12} sm={12}>
                    <Box style={{ height: "68vh", overflowY: "scroll" }}>
                        <Container component="main" maxWidth="lg">
                            <CssBaseline />
                            {data.length !== 0 && <BookmarkVenues data={data} onRemoveBookmark={onRemoveBookmark} />}
