...
export default function AddVenue() {
  ...
  useEffect(() => {
    ...
    axios.get(process.env.REACT_APP_VM_IP + "/app/Fetch_Venue_Types").then(res => {
      if (res.data.status === "OK") {
        ...
        setPropertyType(res.data.data[0])
      } else {
  ...
  const onAddVenue = () => {
    ...
    axios.post(process.env.REACT_APP_VM_IP + "/app/Add_Venue", formData, {
      header: {
      ...
      if (res.data.status === "OK") {
        ...
        setErrorMessage(res.data.messagw)
        setIsErrorSnackBarOpen(true)
      }
    }).catch(e => {
      setErrorMessage("Server Error")
              ...
              <Grid container item xs={12} sm={3} key={idx} className="profileimage1">
                ...
                <div id={"middle" + idx}>
                  <div id={"text" + idx}>
                    ...
                    <input type='file' id={"upload-photo" + idx} onChange={onChangeImage} />
                  </div>
                </div>
              </Grid>
